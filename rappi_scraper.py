...
def extract_menu(page: Page, restaurant_name: str, url: str, log=print) -> RestaurantMenu:
    ...
    # ── Strategy 1: Intercept Rappi API responses ────────────────────────────
    api_responses = []
    ...
    # ── Strategy 2: HTML scraping with popularity badge detection ────────────
    item_selectors = [
    ...
    # Strategy 3: Fallback — extract all text with prices
...
def extract_item_from_card(card) -> Optional[MenuItem]:
    ...
    try:
        ...
        try:
            ...
            if any(kw in outer_lower for kw in ["popular", "most_ordered", "top_seller", "más pedido", "badge"]):
                is_popular = True
        except Exception:
            pass
...
def assign_categories(page: Page, items: list, log=print):
    ...
    try:
        ...
        if most_ordered_count > 0:
            log(f"  ✓ {most_ordered_count} items identificados como 'más pedidos'")

    except Exception as e:
        log(f"  ⚠ No se pudieron asignar categorías: {e}")
            ...
            for name in all_names:
                ...
                if not url:
                    results.append(RestaurantMenu(
                        name=name,
                        city=city,
                        error=f"No se encontró '{name}' en Rappi {city}",
                    ))
                    continue
